    ...
    int		dofold)	    // also recompute the folds
{
	...
	if (wp->w_p_diff && buf_valid(wp->w_buffer))
	{
	    ...
	    n = diff_check(wp, wp->w_topline, NULL);
    ...
    diffio_T   *dio)		// diff output
{
		...
		&& hunk->lnum_orig + hunk->count_orig >= dp->df_lnum[idx_orig])
	{
	    ...
	    {
		dpl = dn->df_next;
		vim_free(dn->df_comparisonlines);
		vim_free(dn);
		dn = dpl;
	    }
	}
...
#endif
		vim_free(dfree->df_comparisonlines);
		vim_free(dfree);
	    }
	    else
 ...
 */
    linenr_T
diff_get_corresponding_line(buf_T *buf1, win_T *win1, linenr_T lnum1)
{
...
f_diff_hlID(typval_T *argvars UNUSED, typval_T *rettv UNUSED)
{
	...
	int linestatus = 0;
	filler_lines = diff_check(curwin, lnum, &linestatus);
